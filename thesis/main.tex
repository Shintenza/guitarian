...
głośnikowej. Urządzenie to widoczne jest na Rysunku \ref{fig:amp}.
\begin{figure}[htbp]
  \centering
...
przełączników, służących do konfiguracji parametrów jego pracy.
  ...
  \item \textbf{Efekty modulacyjne}
    \begin{enumerate}
      \item \textit{Chorus} -- efekt, który przyjmuje sygnał
        ...
        tego sygnału opóźnionymi w czasie.
      \item \textit{Tremolo} -- efekt polegający na okresowej zmianie
        amplitudy sygnału, co przekłada się na jego regularne
      ...
      \item \textit{Phaser} -- efekt, w którym sygnał jest
        ...
        \textit{wirujący} i \textit{przestrzenny} charakter.
    \end{enumerate}
  \item \textbf{Efekty przestrzenne i czasowe}
    \begin{enumerate}
      ...
      \item \textit{Reverb} -- również opiera się na opóźnionych
        ...
        w różnych zamkniętych pomieszczeniach
    \end{enumerate}
  ...
  \item \textbf{Efekty dynamiczne}
    \begin{enumerate}
      \item \textit{Kompresor} -- zmniejsza rozpiętość dynamiczną
        ...
        cichszych, co powoduje wyrównanie dźwięku i wydłużenie
        wybrzmiewania struny (z ang. \textit{sustain})
      \item \textit{Bramka szumów} -- odcina sygnał wejściowy, gdy ten
        spada poniżej ustalonego progu. Skuteczny w redukcji
...
mnogością formatów wtyczek audio. Najbardziej rozpowszechnionym z
nich jest VST (ang. Virtual Studio Technology), opracowany przez
niemiecką firmę \textit{Steinberg Media Technologies}. Choć format
