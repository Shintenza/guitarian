...
niezbędne do pełnego zrozumienia działania systemu. W dalszej kolejności,
zagłębiono się w szczegóły związane z cyfrowym przetwarzaniem sygnału
instrumentalnego. Następnie przeanalizowano architekturę projektu oraz
mechanizmy komunikacji między
poszczególnymi komponentami. W kolejnym rozdziale przybliżono
szczegóły implementacyjne systemu z opisem kluczowych struktur.
Zwieńczeniem pracy jest przejście przez rzeczywiste scenariusze
...
aby za pośrednictwem głośnika ulec konwersji na słyszalną falę
akustyczną\cite{wikipedia_guitar}.
...
Przedwzmacniacz odbiera słaby sygnał i filtruje go, regulując przy
...
odpowiednią barwą, ale nie mający wystarczającej mocy do
poruszenia membranami w kolumnach głośnikowych. Rozwiązaniem tego
właśnie problemu zajmuje się
ten komponent. Może mieć również wpływ na końcowe brzmienie, co jest
szczególnie widoczne w konstrukcjach lampowych. Finalną częścią są kolumny
głośnikowe, których głównym zadaniem jest zamiana sygnału elektrycznego
na fale akustyczne.
  ...
  \item \textbf{Efekty przestrzenne i czasowe}
    \begin{enumerate}
      \item \textit{Delay} -- zapisuje sygnał w buforze i odtwarza go
        ...
        zwrotne, które powoduje serię coraz cichszych
        powtórzeń\cite{realization_of_guitar_audio_effects}.
        Działa podobnie do echa
...
wieloplatformowy i w pełni otwarty standard wtyczek audio.
...
opisują działanie wtyczki. Definiują one między innymi wszystkie
...
co gwarantuje spójność i czytelność metadanych
...
przetłumaczony na postać dyskretną. Urządzeniem
...
struny) do chwili usłyszenia przetworzonego sygnału
akustycznego\cite{audio_latency}. Na
całkowitą wartość RTL składają się następujące
czynniki\cite{audio_latency_components}:
...
\begin{enumerate}
  ...
  \item opóźnienia akustyczne, wynikająca z czasu propagacji fali
...
klasy w połączeniu ze słuchawkami (co pozwala na niemal całkowitą
  eliminację opóźnień
akustycznych), wypadkowe opóźnienie całkowite powinno wynosić w
przybliżeniu 7,5 ms.
...
dedykowanych urządzeń audio na komputerach jednopłytkowych pokroju Raspberry Pi.
...
czasu rzeczywistego, który w stałych odstępach aktywuje
kolejno wszystkich aktywnych, zainteresowanych klientów. Każdy klient
rejestruje własną
...
danym cyklu, zgodnie z połączeniami, gwarantuje deterministyczny
...
najważniejsze – niedeterministyczne. Próba ich wykonania
...
Szczególnym przypadkiem operacji strukturalnych jest usuwanie
wtyczek. Powodują one powstanie osieroconych instancji, które muszą
ulec usunięciu z pamięci. Dealokacja pamięci jest jedną z tych operacji, które
nie powinny mieć miejsca na wątku czasu rzeczywistego, dlatego
instancje usuniętych
wtyczek są odsyłane za pomocą bufora cyklicznego do osobnego wątka,
który zajmuje
się ich utylizacją.
...
W momencie gdy polecenie powiązane jest z usunięciem jednej lub więcej
wtyczek, są one odsyłane dedykowanym buforem cyklicznym do osobnego wątka,
który zajmuje się zwalnianiem pamięci (dealokacja jest kosztowną i
niepotrzebną operacją dla wątka czasu rzeczywistego).
...
\subsubsection{ChainSaver}
Pomocnicza struktura odpowiedzialna za udostępnianie metod
ułatwiających zapis i odczyt zapisanego stanu łańcucha efektów.

\subsubsection{PluginReaper}
Struktura ta inicjalizuje niezależny wątek systemowy w tle, którego
wyłącznym zadaniem jest asynchroniczne nasłuchiwanie na komunikaty
spływające z jednokierunkowego
bufora cyklicznego. Wątek audio, w momencie usunięcia wtyczki z aktywnego
łańcucha efektów, nie niszczy jej instancji bezpośrednio, lecz jedynie
przekazuje ją do wspomnianego bufora. Omawiana struktura na bieżąco odbiera
wyrejestrowane (osierocone) instancje wtyczek i bezpiecznie zwalnia powiązaną
z nimi pamięć operacyjną. Zastosowanie tego mechanizmu
gwarantuje całkowite odizolowanie nieprzewidywalnych, kosztownych operacji
systemowych od krytycznego czasowo toru przetwarzania dźwięku.
...
(jeśli w pamięci są już zapisane szczegóły usługi, krok skanowania
jest pomijany).
...
ładowania wybranego ustawienia lub dokonać jego trwałego usunięcia.
...
środowiskowych z poziomu smartfona. W ramach projektu dostarczono
również skrypty
inicializacyjne, które pozwalają zainstalować system na fizycznym urządzeniu.

Z inżynieryjnego punktu widzenia projekt udowadnia skuteczność
